/**
 * @file consts.cpp
 * @brief Implementation of chemical constants, element properties, and lookup tables.
 * @namespace molgr
 * @author TMJ
 * @date 2025-12-25
 */
...
namespace molgr
{
    ...
    /**
     * @brief Non-Metal Element Dictionary.
     * Key: Atomic Number (int)
     * Value: ElementInfo {Symbol, Name, AtomicNum, Mass, OuterElec, DefaultValence}
     */
    const std::map<int, ElementInfo> kNonMetalDict = {
        {1, {"H", "Hydrogen", 1, 1.008, 1, 1}},
        {2, {"He", "Helium", 2, 4.003, 2, 0}},
        {5, {"B", "Boron", 5, 10.812, 3, 3}},
        {6, {"C", "Carbon", 6, 12.011, 4, 4}},
        {7, {"N", "Nitrogen", 7, 14.007, 5, 3}},
        {8, {"O", "Oxygen", 8, 15.999, 6, 2}},
        {9, {"F", "Fluorine", 9, 18.998, 7, 1}},
        {10, {"Ne", "Neon", 10, 20.18, 8, 0}},
        {14, {"Si", "Silicon", 14, 28.086, 4, 4}},
        {15, {"P", "Phosphorus", 15, 30.974, 5, 3}},
        {16, {"S", "Sulfur", 16, 32.066, 6, 2}},
        {17, {"Cl", "Chlorine", 17, 35.453, 7, 1}},
        {18, {"Ar", "Argon", 18, 39.948, 8, 0}},
        {32, {"Ge", "Germanium", 32, 72.61, 4, 4}},
        {33, {"As", "Arsenic", 33, 74.922, 5, 3}},
        {34, {"Se", "Selenium", 34, 78.96, 6, 2}},
        {35, {"Br", "Bromine", 35, 79.904, 7, 1}},
        {36, {"Kr", "Krypton", 36, 83.798, 8, 0}},
        {51, {"Sb", "Antimony", 51, 121.76, 5, 3}},
        {52, {"Te", "Tellurium", 52, 127.6, 6, 2}},
        {53, {"I", "Iodine", 53, 126.904, 7, 1}},
        {54, {"Xe", "Xenon", 54, 131.29, 8, 0}},
        {84, {"Po", "Polonium", 84, 209.0, 6, 2}},
        {85, {"At", "Astatine", 85, 210.0, 7, 1}},
        {86, {"Rn", "Radon", 86, 222.0, 8, 0}}};

    const std::map<int, double> kNonMetalPaulingElectronegativity = {
        {1, 2.20},
        {5, 2.04},
        {6, 2.55},
        {7, 3.04},
    ...
    const std::map<int, double> kNonMetalFirstIonizationEnergyEv = {
        {1, 13.60},
        {5, 8.30},
        {6, 11.26},
        {7, 14.53},
     ...
     * Value: LennardJonesParams {epsilon, sigma, cutoff}
     */
    const std::map<std::string, LennardJonesParams> kLJParams = {
    ...
    /**
     * @brief D-Orbital Electron Spin Configurations.
     * Represents possible spin states for d-electrons.
     */
    const std::vector<std::vector<int>> kDElectronsSpin = {
        {0},
        {1},
        {2, 0},
        {3, 1},
    ...
    /**
     * @brief Electron configuration (f, d, s, p) for metals.
     * Key: Element Symbol (string)
     * Value: FDSP {f, d, s, p}
     */
    const std::map<std::string, FDSP> kMetalFDSP = {
        {"Li", {0, 0, 1, 0}},
        {"Be", {0, 0, 2, 0}},
        {"Na", {0, 0, 1, 0}},
        {"Mg", {0, 0, 2, 0}},
    ...
    /**
     * @brief Prioritized metal valences.
     * Typically the most common stable oxidation states.
     */
    const std::map<std::string, std::vector<int>> kMetalValencePrior = {
        {"Li", {1}},
        {"Na", {1}},
        {"K", {1}},
        {"Rb", {1}},
    ...
    /**
     * @brief Minor (less common) metal valences.
     */
    const std::map<std::string, std::vector<int>> kMetalValenceMinor = {
        {"Li", {0}},
        {"Na", {0}},
        {"K", {0}},
        {"Rb", {0}},
    ...
    /**
     * @brief Logic function to determine possible radical electrons for a metal atom.
     * * Based on the electron configuration (f, d, s, p) and current valence,
     * determines the number of unpaired electrons (radicals).
     * * @param[in] metal The chemical symbol of the metal (e.g., "Fe").
     * @param[in] valence The oxidation state (valence) of the metal.
     * @return std::set<int> A set of possible radical electron counts.
     */
    std::set<int> GetPossibleMetalRadicals(const std::string &metal, int valence)
    {
        if (kMetalFDSP.find(metal) == kMetalFDSP.end())
        {
            return {};
        }

        const FDSP &val = kMetalFDSP.at(metal);
        int f = val.f;
        int d = val.d;
        int s = val.s;
        int p = val.p;
        ...
        if (valence <= s + p)
        {
            int base = (f + s + p - valence) % 2;
            if (d >= 0 && d < static_cast<int>(kDElectronsSpin.size()))
            {
                for (int dd : kDElectronsSpin[d])
                {
                    result.insert(base + dd);
                }
            }
        }
        else if (valence <= s + p + d)
        {
            int idx = d - valence + s + p;
            if (idx >= 0 && idx < static_cast<int>(kDElectronsSpin.size()))
            {
                for (int dd : kDElectronsSpin[idx])
                {
                    result.insert((f % 2) + dd);
                }
            }
        }
        else if (valence <= s + p + d + f)
        {
            result.insert(f % 2);
        }
