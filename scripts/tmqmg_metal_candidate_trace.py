#!/usr/bin/env python3
"""Trace tmQMg metal-candidate reconstruction by dataset id.
...
def _requested_ids(raw_values: Sequence[str]) -> list[str]:
    ids: list[str] = []
...
def _render_markdown_report(output: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        _markdown_kv_table(
            (
                ("CSV", input_summary.get("csv", "")),
                ("XYZ 目录", input_summary.get("xyz_dir", "")),
                ("id", input_summary.get("ids", [])),
                ("自旋来源", input_summary.get("spin_source", "")),
                ("显式自由基电子数", input_summary.get("total_radical_electrons", "")),
                ("样本数", output.get("case_count", "")),
            )
        ),
    ]
...
def main() -> int:
    args = _parse_args()
    case_ids = _requested_ids(args.ids)
