...
def test_symmetry_low():
    """测试低对称性分子 (甲醇)"""
    score_1 = _core.test_symmetry_penalty("c1ccccc1[O-]")
    score_2 = _core.test_symmetry_penalty("C1=CC=C[C-]C1=O")
...
def test_physchem_charge_penalty():
    ...
    score_na = _core.test_physchem_penalty("[Na+]")

    # [C+] 碳正离子，极其不稳定，应该有高罚分
    score_c_plus = _core.test_physchem_penalty("[C+]")
...
def test_physchem_coulombic_repulsion():
    ...
    score = _core.test_physchem_penalty("[NH3+][NH3+]")
    assert score >= 15.0
...
def test_deviation_tetrahedral_ideal():
    ...
    """
    # C 是第1个原子
    score = _core.test_deviation_score(xyz, 1)
    # 应该是完美的，偏差接近 0
...
def test_total_score_comparison():
    ...
    score_good = _core.test_total_score(xyz_good)
    score_bad = _core.test_total_score(xyz_bad)
