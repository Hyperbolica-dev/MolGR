...
def test_re_dithiocarbamate_variants_do_not_get_artificial_conjugation_gap() -> None:
    ...
    assert abs(
        zwitterionic_metrics.conjugated_atom_count - anionic_metrics.conjugated_atom_count
    ) <= 1
    assert abs(
        zwitterionic_metrics.conjugated_bond_count - anionic_metrics.conjugated_bond_count
    ) <= 1
    assert (
...
def test_au_nhc_zwitterion_variant_keeps_conjugation_component() -> None:
    ...
    assert aromatic_zwitterion_metrics.conjugated_atom_count >= charge_separated_metrics.conjugated_atom_count
